...
namespace CartService.WebApi.IntegrationTests.Controllers.v2
{
	...
	public class CartControllerTests : TestBase
	{
		...
		public async Task GetCartItem_ReturnsOk_WhenItemExists()
		{
			...
			// Act
			var response = await _client.GetAsync($"/api/v2.0/cart/{ValidCartId}/items/{addCartItemModel.CartItemId}");
		...
		{
			// Act
			var response = await _client.GetAsync($"/api/v2.0/cart/{ValidCartId}/items/-1");
		...
		public async Task RemoveItem_ReturnsNoContent_WhenItemExists()
		{
			...
			// Act
			var deleteResponse = await _client.DeleteAsync($"/api/v2.0/cart/{ValidCartId}/items/{addCartItemModel.CartItemId}");
